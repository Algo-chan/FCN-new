...
export class EmailService {
  ...
  private async send(to: string, subject: string, html: string): Promise<void> {
    ...
    await client.emails.send({
      from: "Fast Care Now <noreply@fcncare.com>",
      to,
      subject,
      html
    });
  ...
  async sendOTPEmail(to: string, otp: string, purpose: OtpPurpose): Promise<void> {
    const subject = purpose === "verification" ? "Verify your Fast Care Now account" : "Reset your Fast Care Now password";
    await this.send(
      to,
      subject,
      this.shell(`
        <p style="margin:0 0 16px">Use this one-time code to ${purpose === "verification" ? "verify your Fast Care Now account" : "reset your Fast Care Now password"}.</p>
  ...
  async sendWelcomeEmail(to: string, fullName: string, role: string): Promise<void> {
    await this.send(
      to,
      "Welcome to Fast Care Now - Healthcare Without Walls",
      this.shell(`
        <h2 style="margin:0 0 12px;color:#1E293B">Welcome, ${escapeHtml(fullName)}</h2>
        <p style="margin:0 0 16px">Fast Care Now connects patients, clinicians, and health workers across Ethiopia with remote care, vitals tracking, e-prescriptions, and coordinated follow-up.</p>
        <p style="margin:0 0 20px">Your role is registered as <strong>${escapeHtml(role)}</strong>.</p>
        <a href="${env.FRONTEND_URL}" style="display:inline-block;background:#0A7EA4;color:white;padding:12px 18px;border-radius:8px;text-decoration:none;font-weight:700">Open Fast Care Now</a>
  ...
  async sendHospitalAdminWelcomeEmail(to: string, fullName: string, hospitalName: string, tempPassword: string): Promise<void> {
    await this.send(
      to,
      "You have been added as a Fast Care Now Hospital Administrator",
      this.shell(`
        <h2 style="margin:0 0 12px;color:#1E293B">Welcome, ${escapeHtml(fullName)}</h2>
        ...
        <a href="${env.FRONTEND_URL}/login" style="display:inline-block;background:#0A7EA4;color:white;padding:12px 18px;border-radius:8px;text-decoration:none;font-weight:700;margin-top:12px">Sign In to Fast Care Now</a>
      `)
    );
  }
  ...
  async sendRejectionEmail(to: string, fullName: string, reason: string): Promise<void> {
    await this.send(
      to,
      "Fast Care Now Account Application Update",
      this.shell(`<h2 style="margin:0 0 12px;color:#1E293B">Hello, ${escapeHtml(fullName)}</h2><p>We could not approve your Fast Care Now application at this time.</p><p><strong>Reason:</strong> ${escapeHtml(reason)}</p><p>You may reapply after correcting the issue.</p>`)
    );
  }
  ...
  async sendPasswordChangeAlert(to: string, fullName: string): Promise<void> {
    ...
    await this.send(
      to,
      "Fast Care Now: Your password was changed",
      this.shell(`
        <h2 style="margin:0 0 12px;color:#1E293B">Password Changed</h2>
        <p style="margin:0 0 16px">Your Fast Care Now account password was changed on ${now}.</p>
  ...
  async send2FAEnabledEmail(to: string, fullName: string): Promise<void> {
    await this.send(
      to,
      "2FA has been enabled on your Fast Care Now account",
      this.shell(`
        <h2 style="margin:0 0 12px;color:#1E293B">Two-Factor Authentication Enabled</h2>
        <p style="margin:0 0 16px">Two-factor authentication has been enabled on your Fast Care Now account. Your account is now more secure.</p>
  ...
  async send2FADisabledEmail(to: string, fullName: string): Promise<void> {
    await this.send(
      to,
      "2FA has been disabled on your Fast Care Now account",
      this.shell(`
        <h2 style="margin:0 0 12px;color:#1E293B">Two-Factor Authentication Disabled</h2>
        <p style="margin:0 0 16px">Two-factor authentication has been disabled on your Fast Care Now account.</p>
        <p style="margin:16px 0 0;color:#475569">If you didn't make this change, please contact support immediately.</p>
      `)
    );
  }
    ...
    return `
      <div style="background:#F8FFFE;padding:32px;font-family:Inter,Arial,sans-serif;color:#1E293B">
        <div style="max-width:560px;margin:0 auto;background:#FFFFFF;border:1px solid rgba(10,126,164,0.16);border-radius:12px;overflow:hidden">
          ...
          <div style="padding:16px 24px;background:#F8FFFE;color:#64748B;font-size:12px">Fast Care Now - Healthcare Without Walls</div>
        </div>
      </div>
    `;
  }
