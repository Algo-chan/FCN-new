...
const LoginPage = () => {
  ...
  const redirectUser = (user: { role: string; status: string }) => {
    if (user.role === "admin" || user.role === "super_admin") {
      ...
      }).catch(() => navigate(from ?? "/dashboard", { replace: true }));
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const loginPayload = payload as { user: any; accessToken: string };
      useAuthStore.getState().login(loginPayload.user, { accessToken: loginPayload.accessToken, refreshToken: "" });
      playSuccess();
      redirectUser(loginPayload.user);
    } catch (err: any) {
      const code = err?.response?.data?.error?.code;
      if (code === "GOOGLE_ACCOUNT_ONLY") {
        setGoogleOnly(true);
        setError(null);
      } else if (code === "ACCOUNT_SUSPENDED") setError({ type: "danger", message: err?.response?.data?.error?.message ?? "Account suspended" });
  ...
  const handleVerifyOTP = async (otp: string) => {
    ...
    setOtpError(null);
    try {
      const { data: response } = await authService.loginVerifyOTP(otpEmail, otp);
      const payload = response.data;
      if (!payload) throw new Error(response.error?.message ?? "Verification failed");
  ...
  const handleResendOTP = async () => {
    await authService.resendOTP(otpEmail, "login");
  };
          ...
          <AnimatePresence mode="wait">
            {!requiresOTP ? (
              <motion.div key="login-form" initial={!shouldReduceMotion ? { opacity: 0, x: 20 } : undefined} animate={{ opacity: 1, x: 0 }} exit={!shouldReduceMotion ? { opacity: 0, x: -20 } : undefined}>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  <AnimatePresence>
                    ...
                    {googleOnly && (
                      <motion.div initial={!shouldReduceMotion ? { opacity: 0, y: -10 } : undefined} animate={{ opacity: 1, y: 0 }} exit={!shouldReduceMotion ? { opacity: 0, y: -10 } : undefined} className="rounded-md bg-fcn-accent/10 px-4 py-3 text-sm text-fcn-accent">
                        <p className="font-medium">This account was created with Google.</p>
                        <p className="mt-1 text-fcn-accent/70">Please use the <strong>Sign in with Google</strong> button below.</p>
                      </motion.div>
                    )}
                  </AnimatePresence>
              ...
              <OTPVerification
                ...
                onBack={otpLocked ? undefined : handleBackToLogin}
