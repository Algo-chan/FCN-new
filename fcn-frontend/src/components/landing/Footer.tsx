              ...
              <div className="mt-4 flex gap-3">
                {socialIcons.map((s, i) => (
                  <motion.a
                    key={i}
                    href={s.href}
                    whileHover={shouldReduceMotion ? {} : { scale: 1.2, rotate: 5, color: "#2DD4BF" }}
                    whileTap={shouldReduceMotion ? {} : { scale: 0.9 }}
                    className="rounded-full p-2 text-fcn-text-light/40 transition-colors dark:text-gray-400"
                  ...
                  {col.items.map((item) => (
                    <li key={item.label}>
                      <motion.a
                        ...
                        className="inline-block text-sm text-fcn-text-light/50 transition-colors hover:text-fcn-accent dark:text-gray-400"
                      >
                        {item.label}
                      </motion.a>
                    </li>
            ...
            <div>
              ...
              <div className="flex flex-col gap-2 sm:flex-row">
                <input
                  ...
                  className="h-10 flex-1 rounded-lg border border-fcn-primary/20 bg-white px-3 text-sm text-fcn-text-light outline-none transition-all focus:border-fcn-accent focus:ring-2 focus:ring-fcn-accent/30 dark:bg-[#0D1117] dark:text-fcn-text-dark"
                  aria-label="Email for newsletter"
                />
                <motion.button
                  whileHover={shouldReduceMotion ? {} : { scale: 1.03 }}
                  whileTap={shouldReduceMotion ? {} : { scale: 0.97 }}
                  className="rounded-lg bg-fcn-primary px-3 text-sm font-medium text-white transition-colors hover:bg-fcn-primary/80"
                >
                  Subscribe
                </motion.button>
              </div>
